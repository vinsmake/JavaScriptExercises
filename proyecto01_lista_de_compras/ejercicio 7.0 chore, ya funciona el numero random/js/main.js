...
//Inicio boton///////////////////////
botonAgregar.addEventListener ("click", function(e) { //inicio funcion de boton.
    e.preventDefault();
...
//fin json////////////////////////
//Inicio if ////////////////////////////////
tabla[0].innerHTML = ""; //Se borra lo que ya se tiene para no imprimirse dos veces. 
if (localStorage.getItem(key)){
    let tmp = JSON.parse(localStorage.getItem(key));
//metodo del profe///////////////////////
    arreglo = tmp ///la verdad no se por que estaba eso
    id = arreglo.length+1; //Se supone que es para guardar lo que se tenia antes (mas bien, el id)
//fin metodo del profe///////////////////////
    //recorre cada uno de los elementos y con ellos puede leer su valor.
    tmp.forEach(element => {     // funcion arrow
        tabla[0].innerHTML +=
            ` 
     <tr>
